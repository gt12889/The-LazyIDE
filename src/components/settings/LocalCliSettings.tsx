import { OpenCodeGoSettings } from './OpenCodeGoSettings';
...
export function LocalCliSettings() {
  ...
  return <section style={{ maxWidth: 720, display: 'grid', gap: 20 }}>
    ...
    <p>Use Ollama or LM Studio on this computer, or a signed-in CLI in the desktop app. Local and CLI engines need no lazygt account. OpenCode Go uses your own subscription key.</p>
    <label>Engine<select style={field} value={mode} onChange={e => setMode(e.target.value as 'local' | 'cli' | 'opencode-go')}>
      <option value="opencode-go" disabled={!isTauri()}>OpenCode Go subscription</option><option value="local">Local model</option><option value="cli" disabled={!isTauri()}>CLI (desktop app)</option>
    </select></label>
    {mode === 'opencode-go' ? <OpenCodeGoSettings /> : mode === 'local' ? <>
    ...
    {mode !== 'opencode-go' && <button onClick={() => { try { persist(); window.location.reload(); } catch (e) { setStatus(String(e)); } }}>Save and apply</button>}
