...
export type EngineReadiness = { mode: 'opencode-go' | 'cli' | 'byok' | 'pro' | 'local'; ready: boolean; reason?: EngineReadinessReason };
...
export function getEngineReadiness(forMode?: EngineReadiness['mode'], _modelId?: string): EngineReadiness {
 ...
 if (mode === 'opencode-go') return { mode, ready: describeProviderReadiness('opencode-go').ready };
